...
int main(void){
    ...
    printf("Type in a periodicity in (ms) of each thread \n");
    scanf("%lf", &periodicity);

	printf("Type in a number of thread disturbator \n");
	scanf("%d", &number_thread);

    per_100 = (unsigned int)(periodicity * 1000);

    //memset(&num_thread, '\0', sizeof(num_thread));
